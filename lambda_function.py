...
def process_xml_records(content):
    ...
    for cust in root.findall(".//Customer"):
        ...
        if not city.text or city.text.strip() == "":
            invalid_customers.append(cust)
            continue
        valid_customers.append(cust)  # Add valid customer to the list
...
def build_xml(customers):
    ...
    for cust in customers:
        root.append(cust)  # Append each customer to the XML root
    return ET.tostring(root, encoding="utf-8")  # Convert to string

# Process JSON records to validate and separate valid and invalid records
def process_json_records(content):
    records = json.loads(content)  # Parse JSON content
    valid = []
    invalid = []
...
def process_csv_records(content):
    ...
    for row in reader:
        try:
            ...
            invalid.append(row)  # Add to invalid if an error occurs
...
def lambda_handler(event, context):
    ...
    try:
        obj = s3.get_object(Bucket=bucket, Key=key)  # Retrieve the object from S3
        content = obj["Body"].read().decode("utf-8")  # Read and decode the content
    except Exception as e:
